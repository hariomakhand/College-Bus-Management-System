...
import { Bus, UserCheck, MapPin, Users, BarChart3, Activity, TrendingUp, CheckCircle } from 'lucide-react';
...
const Dashboard = () => {
  // RTK Query hooks - All data from backend
  ...
  // Debug logging
  console.log('Dashboard Students Data:', { students, studentsLoading, studentsError });
  ...
  console.log('Students Array:', studentsArray, 'Length:', studentsArray.length);
  ...
  // Dynamic stats array with real backend data
  const statsArray = [
    { 
      title: 'Total Buses', 
      value: totalBuses, 
  ...
  // Dynamic Pie Chart - Bus Status with real data
  const pieData = {
  ...
  // Dynamic Line Chart - Growth trend with real data
  const currentMonth = new Date().getMonth();
  const months = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'];
  const last6Months = months.slice(Math.max(0, currentMonth - 5), currentMonth + 1);
  
  // Generate growth data based on current totals
  ...
  // Show error state if critical API calls failed (students is optional)
    ...
    return (
      <div className="bg-red-50 border border-red-200 rounded-lg p-6 text-center">
        ...
        <div className="text-sm text-red-500 mb-4">
          ...
          {busesError && <div>Buses: {busesError.message}</div>}
          {driversError && <div>Drivers: {driversError.message}</div>}
          {routesError && <div>Routes: {routesError.message}</div>}
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Dynamic Charts Section - Single Line Layout */}
      ...
      {/* Real-time Activities and Quick Stats */}
